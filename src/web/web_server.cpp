...
void WebServer::run() {
    ...
    mDatabaseWorker->asyncRegisterUser("svendcsvendsen@gmail.com", "password", [this](bool success, const Token &token) {
        log_debug().field("success", success).msg("register user callback");
        if (!success)
            return;
        mDatabaseWorker->asyncRequestToken("svendcsvendsen@gmail.com", "password", [this](bool success, const Token &token) {
            log_debug().field("success", success).msg("request token callback");
            if (!success)
                return;
            mDatabaseWorker->asyncValidateToken("svendcsvendsen@gmail.com", token, [](bool success) {
